...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,goal=GOAL_SCORE, say=silence):
    ...
    while score0<goal and score1<goal:
        ...
        "*** YOUR CODE HERE ***"
        if turn ==0:
           com=say(score0,score1)
        else:
           com=com(score0,score1)
        turn=turn + 1
        # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max=0
    for i in range(1,11):
        tmp=sum([roll_dice(i,dice) for k in range(1,trials_count+1)])/trials_count
        if max<tmp:
            num=i
            max=tmp
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    nums_rolls=max_scoring_num_rolls(six_sided,1000)
    if score>opponent_score:
       return nums_rolls
    else:
        return extra_turn_strategy(score,opponent_score,8,nums_rolls)
