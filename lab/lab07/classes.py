...
class Player:
    ...
    def play(self, card_index):
        ...
        """
        "*** YOUR CODE HERE ***"
        assert len(self.hand)!=0,"without cards in your hand"
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        player.hand+=[other_card]
        for i in range(len(opponent.deck.cards)):
            if opponent.deck.cards[i].attack==other_card.attack and opponent.deck.cards[i].defense==other_card.defense:
