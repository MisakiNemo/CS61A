...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(min([min_city for min_city in [city_a,city_b]],key=lambda x:distance(x,ct)))
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        return tree(label(t1)+label(t2),[add_trees(s1,s2) for s1,s2 in zip(branches(t1),branches(t2))])
